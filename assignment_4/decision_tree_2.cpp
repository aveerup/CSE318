...
//   mt19937 gen(42);                       
  ...
  while(getline(fin, line)){
    if(line_no == 0 ){
      ...
      if(line_no%4 == randomNum){
        train_data_file<<line<<endl;
      }else{
        test_data_file<<line<<endl;
      }
      
      if(line_no != 0 && line_no%4 == 0){
        randomNum = dist(gen);
      }
...
//   }
    ...
    if (argc != 3) {
          cout << "Usage: " << argv[0] << " <number_of_trees> <maxDepth>" << endl;
          cout << "Criterion: IG, IGR, or NWIG" << endl;
          cout << "MaxDepth: 0 for no pruning, positive integer for depth limit" << endl;
          return 1;
      }
  ...
  string criterion = "IG";
  int number_of_trees = stoi(argv[1]);
  ...
  for (int run = 0; run < number_of_trees; run++) {
